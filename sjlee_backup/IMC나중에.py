...
from sjlee_backup.loss import loss_superglue
    ...
    def forward(self, data):
        ...
        with torch.no_grad():
            ...
            if m + n < max_keypoints *2:
                ...
                scores = F.pad(scores, p2d, 'constant', 0.).type(scores.dtype)

        
        scores = self.decoder(scores[:, None, :, :])
        scores = scores[:, :m, :n]
        ...
        scores[scores>thr] = thr
        #print(scores)
        scores = (softmax_with_temperature(scores))
        ...
        scores = scores[:, :-1, :-1].exp()
        '''
        
        #print(scores)
        
        #print(scores.min(), scores.max())
        #print(scores.exp().min(), scores.exp().max())
