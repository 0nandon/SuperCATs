...
parser.add_argument(
    ...
    help='batch_size')
parser.add_argument(
    '--train_path', type=str, default='/home/cvlab09/projects/seungjun_an/dataset/indoor2/', 
...
if __name__ == '__main__':
    ...
    optimizer = torch.optim.Adam(matching.parameters(), lr=opt.learning_rate)
    ...
    for epoch in range(1, opt.epoch+1):
        ...
        for i in range(numOftrainSet):
            ...
            if str(type(kp1_projected[0])) != '<class \'numpy.ndarray\'>' or str(type(kp2_np[0])) !='<class \'numpy.ndarray\'>' : 
                print(str(type(kp1_projected[0])))
                continue
            ...
            Loss = loss_superglue(scores, all_matches.permute(1, 2, 0))
            #print(Loss)
            ...
            if (i+1) % test == 0:      ############################50
                print ('Epoch [{}/{}], Step [{}/{}], Loss: {:.4f}' 
                    .format(epoch, opt.epoch, i+1, numOftrainSet, sum_loss / float(test))) 
                sum_loss = 0.
                ...
                print('superglue->train')################################
                #matching.float().train()#################################
            # process checkpoint for every 5e3 images
            if (i+1) % 1000 == 0:
                model_out_path = "indoor_state_dict_epoch{}.pth".format(epoch)
                torch.save(matching.state_dict(), model_out_path)
                #torch.save(matching, model_out_path)
                #torch.save(model_state_dict(),)
                print ('Epoch [{}/{}], Step [{}/{}], Checkpoint saved to {} ' 
                    .format(epoch, opt.epoch, i+1, numOftrainSet, model_out_path)) 
                import sys;sys.exit()
            
        # save checkpoint when an epoch finishes
        epoch_loss /= numOftrainSet
        model_out_path = "indoor_state_dict_epoch{}.pth".format(epoch)
        torch.save(matching.state_dict(), model_out_path)
        #torch.save(matching, model_out_path)
    ...
    print(pred['matches0'].size())

    print(pred['matches1'].size())


    print(pred['matches0'].size())
    print(pred['matches1'].size())
    print(pred['matching_scores0'].size())
    print(pred['matching_scores1'].size())
